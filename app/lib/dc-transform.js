...
const FINDERS = `
  ...
  var PHONE_WIDTH_FIXES = [
    ...
    /* Two reorderings, and both reverse something the design states on purpose.

       The design pushes the filter sidebar below the results on a narrow
       screen — [data-r~="fside"] { order: 2 } — and pulls the buy card above
       the gallery on a detail page — [data-r~="dside"] { order: -1 }. So on a
       phone the market opened with 412 listings and the filters two thousand
       pixels underneath, and the car listing led with its name and price while
       the photograph sat below them. The client asked for the opposite of both:
       filters above the listings, the picture above the name.

       Keyed on [data-narrow] rather than a width, so they switch exactly when
       the design's own rules do. The extra weight is deliberate — the rules
       being overridden are already !important at (1,3,1).

       These belong in the design. They are not gaps being filled, they are
       decisions being reversed, and leaving them here means the design and the
       published demo disagree about what a phone should show. */
    'html #webAppScaleBox[data-narrow="1"] [data-r~="stack"] > [data-r~="fside"] {',
    '  order: -1 !important;',
    '}',

    /* Putting the photograph first takes more than an order, because it is not
       a child of the row — it is the first thing inside dmain, which is the
       column that also holds the description, the details table and the map.
       display: contents drops that column's box so its children join the row
       directly and can be ordered against the buy card, without a single node
       being moved.

       The photograph and its thumbnail strip go to -2, ahead of the buy card
       at the -1 the design already gives it; everything else in dmain keeps
       the default 0 and lands after. Reading order becomes photo, thumbnails,
       name and price, buy, then the description.

       dmain\'s own 18px gap goes with its box, so the row\'s 40px would apply
       between every one of those. The row gets the tighter gap back. */
    'html #webAppScaleBox[data-narrow="1"] [data-r="dmain"] { display: contents !important; }',
    'html #webAppScaleBox[data-narrow="1"] [data-r~="stack"]:has(> [data-r="dmain"]) {',
    '  gap: 18px !important;',
    '}',
    'html #webAppScaleBox[data-narrow="1"] [data-r="dmain"] > :nth-child(1),',
    'html #webAppScaleBox[data-narrow="1"] [data-r="dmain"] > [data-r="g4"] {',
    '  order: -2 !important;',
    '}',
    /* Four thumbnails reflowed to two columns ran 374px tall — taller than the
       photograph they belong to, and now sitting between it and the price.
       Kept as one four-across strip, scoped to the gallery so the design\'s
       other four-card rows go on reflowing. */
    'html #webAppScaleBox[data-narrow="1"] [data-r="dmain"] > [data-r="g4"] {',
